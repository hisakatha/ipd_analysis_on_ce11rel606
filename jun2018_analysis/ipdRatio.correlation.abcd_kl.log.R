...
# Sample ABCD
abcd_name <- "ABCD_WGA"
abcd_data <- h5file(name = "/glusterfs/hisakatha/methylation/smrtpipe/vs_ce11rel606/jun2018_abcd_pcr/output/tasks/kinetics_tools.tasks.gather_kinetics_h5-1/file.h5", mode = "r");
# Sample KL
kl_name <- "KL_native"
kl_data <- h5file(name = "/glusterfs/hisakatha/methylation/smrtpipe/vs_ce11rel606/jun2018_kl_nopcr/output/tasks/kinetics_tools.tasks.gather_kinetics_h5-1/file.h5", mode = "r");
...
### abcd vs. kl
result1 <- compare_between_two(abcd_data, abcd_name, kl_data, kl_name, whole_targets_chr, whole_targets_start, whole_targets_end, thresh, 0);

invisible(dev.off());

#pdf(sprintf("ipdRatio.correlation.compact.all.cov%d_%d_%d.v2.pdf", thresh1, thresh2, thresh3), width = 14, height = 18, onefile = TRUE);
pdf(sprintf("ipdRatio.correlation.compact.abcd_kl.cov%d.pdf", thresh), width = pdf_width, height = pdf_height, onefile = TRUE);
