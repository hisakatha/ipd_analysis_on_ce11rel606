...
get_motif_ipd <- function(data, occ, out){
    # data: kinetics data
    ...
    for (i in 1:occ_n){
        ...
        out_values <- c(rep(0, (collect_start - index_start) * 2), data[occ_chr][(collect_start * 2 + 1):(collect_end * 2 + 2), tMean], rep(0, (index_end - collect_end) * 2))
...
occ <- fread(occ_file, sep = " ", col.names = merged_occ_col)

out_prefix <- paste0(occ_file, ".ipd")
get_motif_ipd(ab_data, occ, paste0(out_prefix, ".ab.csv"))
get_motif_ipd(cd_data, occ, paste0(out_prefix, ".cd.csv"))
get_motif_ipd(k_data, occ, paste0(out_prefix, ".k.csv"))
get_motif_ipd(l_data, occ, paste0(out_prefix, ".l.csv"))
