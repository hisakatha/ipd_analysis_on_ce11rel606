...
plot_motif_kinetics <- function(kinetics, title_text, ylab_text, global_mean, global_var, global_size){
    ...
    g1 <- ggplot(kinetics_summary[strand == "+" & position >= 9 & position <= 13], aes(position, mean)) + geom_point() +
...
pdf(paste0(input_prefix, ".simple.pdf"), width = 2, height = 2)
plot_motif_kinetics(ab_ipd, paste0("sample AB"), bquote(log[2]~IPD), ab_log2ipd_mean, ab_log2ipd_var, ab_log2ipd_size)
plot_motif_kinetics(cd_ipd, paste0("sample CD"), bquote(log[2]~IPD), cd_log2ipd_mean, cd_log2ipd_var, cd_log2ipd_size)
plot_motif_kinetics(k_ipd, paste0("sample K"), bquote(log[2]~IPD), k_log2ipd_mean, k_log2ipd_var, k_log2ipd_size)
plot_motif_kinetics(l_ipd, paste0("sample L"), bquote(log[2]~IPD), l_log2ipd_mean, l_log2ipd_var, l_log2ipd_size)
