...
get_motif_ipdratio <- function(data, occ, out, ref_skips){
    ...
    for (i in 1:occ_n){
        ...
        out_values <- c(rep(0, (collect_start - index_start) * 2), data[ref_skips[occ_chr] + (collect_start * 2 + 1):(collect_end * 2 + 2), ipdRatio], rep(0, (index_end - collect_end) * 2))
...
source("../../load_jun2018_data.saved.fst.basic_normalized.R", chdir = TRUE)

#bed_col <- c("chr", "start", "end", "name", "score", "strand")
merged_occ_col <- c("chr", "start", "strand")

k_normBy_ab_occ <- fread(paste0("motif_occ.deep_kinetics_region.k_normBy_ab.bed.slop20.bed.fa.bed.merged_occ"), sep = " ", col.names = merged_occ_col)
l_normBy_cd_occ <- fread(paste0("motif_occ.deep_kinetics_region.l_normBy_cd.bed.slop20.bed.fa.bed.merged_occ"), sep = " ", col.names = merged_occ_col)

reference_skip <- c(0, cumsum(width(reference)) * 2)
names(reference_skip) <- names(reference)

get_motif_ipdratio(k_normBy_ab_data, k_normBy_ab_occ, "motif_ipdratio.k_normBy_ab.csv", reference_skip)
get_motif_ipdratio(l_normBy_cd_data, l_normBy_cd_occ, "motif_ipdratio.l_normBy_cd.csv", reference_skip)
