...
l_occ <- fread(paste0("motif_occ.deep_kinetics_region.l.bed.merged.sorted.slop20.bed.fa.bed.merged_occ"), sep = " ", col.names = merged_occ_col)

reference_skip <- c(0, cumsum(width(reference)) * 2)
names(reference_skip) <- names(reference)

get_motif_ipd(l_data, l_occ, "motif_ipd.l.csv", reference_skip)
