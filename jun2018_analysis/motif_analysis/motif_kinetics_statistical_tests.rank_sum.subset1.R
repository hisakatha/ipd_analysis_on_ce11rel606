...
source("../load_jun2018_data.saved.fst.R", chdir = TRUE)
...
foreach_ret <- foreach(i = 1:length(subset1_motifs)) %do% {
    ...
    output <- sprintf("motif_kinetics_statistical_tests.rank_sum.subset1.%s.csv", motif_dir)
...
fwrite(rbindlist(foreach_ret), file = "motif_kinetics_statistical_tests.rank_sum.subset1.csv")
