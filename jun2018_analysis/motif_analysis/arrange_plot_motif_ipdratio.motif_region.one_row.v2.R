...
ab_log2ipdratio_mean <- 0.0281014985112332
ab_log2ipdratio_var <- 0.197214
ab_log2ipdratio_size <- 26968723
cd_log2ipdratio_mean <- 0.0282598097559704
cd_log2ipdratio_var <- 0.199136
cd_log2ipdratio_size <- 45958624
PD2182_log2ipdratio_mean <- -0.0116997125975452
PD2182_log2ipdratio_var <- 0.164946
PD2182_log2ipdratio_size <- 1356981
PD2182sequel_log2ipdratio_mean <- 0.044524459442385
PD2182sequel_log2ipdratio_var <- 0.322242
...
for (motif_dir in motif_dirs) {
    ab_ipdratio <- fread(paste0(motif_dir, "/motif_ipdratio.ab.c_elegans.csv"))
    ab_plots[[motif_dir]] <- plot_motif_kinetics(ab_ipdratio, lookup_motif_title(motif_dir), ylab_expr, ab_log2ipdratio_mean, ab_log2ipdratio_var, ab_log2ipdratio_size) + theme(legend.position = "none") + coord_cartesian(ylim = ylim_ipd)
    cd_ipdratio <- fread(paste0(motif_dir, "/motif_ipdratio.cd.c_elegans.csv"))
    cd_plots[[motif_dir]] <- plot_motif_kinetics(cd_ipdratio, lookup_motif_title(motif_dir), ylab_expr, cd_log2ipdratio_mean, cd_log2ipdratio_var, cd_log2ipdratio_size) + theme(legend.position = "none") + coord_cartesian(ylim = ylim_ipd)
    PD2182_ipdratio <- fread(paste0(motif_dir, "/motif_ipdratio.PD2182.c_elegans.csv"))
    PD2182_plots[[motif_dir]] <- plot_motif_kinetics(PD2182_ipdratio, lookup_motif_title(motif_dir), ylab_expr, PD2182_log2ipdratio_mean, PD2182_log2ipdratio_var, PD2182_log2ipdratio_size) + theme(legend.position = "none") + coord_cartesian(ylim = ylim_ipd)
    PD2182sequel_ipdratio <- fread(paste0(motif_dir, "/motif_ipdratio.PD2182sequel.c_elegans.csv"))
    PD2182sequel_plots[[motif_dir]] <- plot_motif_kinetics(PD2182sequel_ipdratio, lookup_motif_title(motif_dir), ylab_expr, PD2182sequel_log2ipdratio_mean, PD2182sequel_log2ipdratio_var, PD2182sequel_log2ipdratio_size) + theme(legend.position = "none") + coord_cartesian(ylim = ylim_ipd)
...
pdf_width <- 10
pdf_height <- 4
pdf("arrange_plot_motif_ipdratio.stderr.wide_y.motif_region.one_row.v2.ab.pdf", width = pdf_width, height = pdf_height)
...
invisible(dev.off())
pdf("arrange_plot_motif_ipdratio.stderr.wide_y.motif_region.one_row.v2.cd.pdf", width = pdf_width, height = pdf_height)
...
invisible(dev.off())
pdf("arrange_plot_motif_ipdratio.stderr.wide_y.motif_region.one_row.v2.PD2182.pdf", width = pdf_width, height = pdf_height)
...
invisible(dev.off())
pdf("arrange_plot_motif_ipdratio.stderr.wide_y.motif_region.one_row.v2.PD2182sequel.pdf", width = pdf_width, height = pdf_height)
