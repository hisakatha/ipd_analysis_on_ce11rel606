...
get_motif_ipdratio <- function(data, occ, out){
    ...
    for (i in 1:occ_n){
        ...
        out_values <- c(rep(0, (collect_start - index_start) * 2), data[occ_chr][(collect_start * 2 + 1):(collect_end * 2 + 2), ipdRatio], rep(0, (index_end - collect_end) * 2))
...
get_motif_ipdratio(k_normBy_ab_data, k_normBy_ab_occ, "motif_ipdratio.k_normBy_ab.csv")
get_motif_ipdratio(l_normBy_cd_data, l_normBy_cd_occ, "motif_ipdratio.l_normBy_cd.csv")
