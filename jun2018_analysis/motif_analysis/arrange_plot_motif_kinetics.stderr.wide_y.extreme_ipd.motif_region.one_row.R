...
for (motif_dir in motif_dirs) {
    ...
    ab_plots[[motif_dir]] <- plot_motif_kinetics(ab_ipd, motif_dir, ylab_expr, ab_log2ipd_mean, ab_log2ipd_var, ab_log2ipd_size) + theme(legend.position = "none") + coord_cartesian(ylim = ylim_ipd)
    cd_ipd <- fread(paste0(motif_dir, "/motif_ipd.cd.csv"))
    cd_plots[[motif_dir]] <- plot_motif_kinetics(cd_ipd, motif_dir, ylab_expr, cd_log2ipd_mean, cd_log2ipd_var, cd_log2ipd_size) + theme(legend.position = "none") + coord_cartesian(ylim = ylim_ipd)
    PD2182_ipd <- fread(paste0(motif_dir, "/motif_ipd.PD2182.csv"))
    PD2182_plots[[motif_dir]] <- plot_motif_kinetics(PD2182_ipd, motif_dir, ylab_expr, PD2182_log2ipd_mean, PD2182_log2ipd_var, PD2182_log2ipd_size) + theme(legend.position = "none") + coord_cartesian(ylim = ylim_ipd)
    PD2182sequel_ipd <- fread(paste0(motif_dir, "/motif_ipd.PD2182sequel.csv"))
    PD2182sequel_plots[[motif_dir]] <- plot_motif_kinetics(PD2182sequel_ipd, motif_dir, ylab_expr, PD2182sequel_log2ipd_mean, PD2182sequel_log2ipd_var, PD2182sequel_log2ipd_size) + theme(legend.position = "none") + coord_cartesian(ylim = ylim_ipd)
...
pdf_width <- 16
pdf_height <- 4
pdf("arrange_plot_motif_kinetics.stderr.wide_y.motif_region.one_row.ab.pdf", width = pdf_width, height = pdf_height)
...
invisible(dev.off())
pdf("arrange_plot_motif_kinetics.stderr.wide_y.motif_region.one_row.PD2182.pdf", width = pdf_width, height = pdf_height)
...
invisible(dev.off())
pdf("arrange_plot_motif_kinetics.stderr.wide_y.motif_region.one_row.PD2182sequel.pdf", width = pdf_width, height = pdf_height)
