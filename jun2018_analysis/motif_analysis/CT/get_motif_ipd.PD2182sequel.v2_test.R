...
get_motif_ipd <- function(data, occ, out, ref_skips){
    ...
    cat(sprintf("length(table_list) = %g\n", length(table_list)))
    cat(sprintf("occ_n = %g\n", occ_n))
    cat("nrow of table_list:\n")
    print(paste(lapply(table_list[1:500], nrow)))
...
cat(sprintf("Expected occ_n: %d\n", PD2182sequel_occ[,.N] %/% 20))
get_motif_ipd(PD2182sequel_data, PD2182sequel_occ[1:(.N %/% 20)], "motif_ipd.test.PD2182sequel.csv", reference_skip)
