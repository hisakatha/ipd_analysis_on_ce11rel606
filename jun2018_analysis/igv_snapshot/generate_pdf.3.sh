...
OUT=get_enriched_genes.both.igv_snapshot.positive_enrichment.tex
cat <<'_EOS_' > $OUT
...
gene_order="12 1 18 24 23 25 16 17 19 20 $(seq 2 11) 13 14 15 21 22 $(seq 26 36)"
gene_order_index=$(echo $gene_order | xargs -n1 | xargs -I{} bash -c 'echo $(({} - 1))')
for i in $gene_order_index
do
    i1=$((2 * i))
    i2=$((2 * i + 1))
    GENE=${genes[$i1]}
    ...
    pvalue2=${pvalues[$i2]}
    qvalue2=${qvalues[$i2]}
    if [[ $(echo "$enrichment1 > 1.0 && $enrichment2 > 1.0" | bc) != 1 ]]; then continue; fi
