...
pre_csv="count_occ.celegans_and_ecoli.set1.pre_csv"
...
do
    ...
    do
        ../count_occ.celegans_and_ecoli.sh $sample $n_motifs_high_ipd $n_motifs_low_ipd || exit 1
    done
    cd ../
done > $pre_csv

(cat $pre_csv | grep sample_id | head -n1; cat $pre_csv | grep -v "#" | grep -v sample_id) > count_occ.celegans_and_ecoli.set1.csv
