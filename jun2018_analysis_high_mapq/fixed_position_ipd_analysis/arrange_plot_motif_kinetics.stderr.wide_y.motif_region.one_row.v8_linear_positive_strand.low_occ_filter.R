...
correlation_output_prefix <- "arrange_plot_motif_kinetics.stderr.wide_y.motif_region.one_row.v8_linear_positive_strand.low_occ_filter.correlation"
...
plot_motif_kinetics_comparison <- function(kinetics1_summary, kinetics2_summary, type1, type2, title_text, motif_string, sample_name, ylab_text, global_mean, global_var, global_size){
    ...
    # g1
    plot_data <- kinetics1_summary[region == "Motif"]
    annot_x_rate <- 0.7
    ...
    annot_x <- quantile(c(kinetics1_summary[, min(position)], kinetics1_summary[, max(position)]), annot_x_rate, names = FALSE)
    g1_outside20 <- g1 %+% kinetics1_summary + geom_vline(xintercept = c(0.5, end_pos - 19.5), linetype = "dashed", size = 0.1) +
        theme(axis.text.x = element_text(size = 5))
    # g1_outside10
    plot_data <- kinetics1_summary[-9 <= position & position <= end_pos - 10]
    annot_x <- quantile(c(plot_data[, min(position)], plot_data[, max(position)]), annot_x_rate, names = FALSE)
