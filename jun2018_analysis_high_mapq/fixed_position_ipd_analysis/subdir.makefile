...
fixed_pos := fixed_position.gff
$(occ_prefix).ce11rel606.bed: $(REF).bed
	bedtools intersect -s -a $< -b $(fixed_pos) -F 1 -nonamecheck -sorted -g $(FAI) > $@ && touch $@
# Root directory for sample analyses
#DIR := /glusterfs/hisakatha/methylation/smrtpipe/vs_ce11rel606
DIR := ../../..
FILE := mapped.alignmentset.merged.high_mapq.bam.cov25.slop20.bed
...
$(occ_prefix).PD2182sequel.bed: $(DIR)/PD2182-sequel_no_chunk/$(FILE)
	bedtools intersect -s -a $< -b $(fixed_pos) -F 1 -nonamecheck -sorted -g $(FAI) > $@ && touch $@
$(occ_prefix).k_normBy_ab.bed: $(DIR)/jun2018_analysis_high_mapq/deep_region_cov25.k_normBy_ab.slop20.bed
	bedtools intersect -s -a $< -b $(fixed_pos) -F 1 -nonamecheck -sorted -g $(FAI) > $@ && touch $@
$(occ_prefix).l_normBy_cd.bed: $(DIR)/jun2018_analysis_high_mapq/deep_region_cov25.l_normBy_cd.slop20.bed
	bedtools intersect -s -a $< -b $(fixed_pos) -F 1 -nonamecheck -sorted -g $(FAI) > $@ && touch $@
$(occ_prefix).abcd.bed: $(DIR)/jun2018_abcd_pcr_no_chunk/$(FILE)
	bedtools intersect -s -a $< -b $(fixed_pos) -F 1 -nonamecheck -sorted -g $(FAI) > $@ && touch $@
$(occ_prefix).kl.bed: $(DIR)/jun2018_kl_nopcr_no_chunk/$(FILE)
	bedtools intersect -s -a $< -b $(fixed_pos) -F 1 -nonamecheck -sorted -g $(FAI) > $@ && touch $@
$(occ_prefix).kl_normBy_abcd.bed: $(DIR)/jun2018_analysis_high_mapq/deep_region_cov25.kl_normBy_abcd.slop20.bed
	bedtools intersect -s -a $< -b $(fixed_pos) -F 1 -nonamecheck -sorted -g $(FAI) > $@ && touch $@

deep_kinetics_beds := $(wildcard ../../deep_kinetics_region.*.slop20.bed)
occ_deep_kinetics := $(deep_kinetics_beds:../../%=$(occ_prefix).%.bed)
$(occ_deep_kinetics): $(occ_prefix).%.bed: ../../%
	bedtools intersect -s -a $< -b $(fixed_pos) -F 1 -nonamecheck -sorted -g $(FAI) > $@ && touch $@

# fixed_position near features (such as DNA modification and high IPD)
occ_feature_gff := $(wildcard ../../*.slop20.fullLength.gff)
occ_feature_bed := $(occ_feature_gff:=.sorted.bed)
occ_feature := $(occ_feature_bed:../../%=$(occ_prefix).%.bed)
...
$(motif_ipds): get_motif_ipd.R.log
	touch $@
get_motif_ipd.R.log: $(occ_prefix).deep_kinetics_region.ab.bed.merged.sorted.slop20.bed.bed.merged_occ $(occ_prefix).deep_kinetics_region.cd.bed.merged.sorted.slop20.bed.bed.merged_occ
	Rscript ../get_motif_ipd.v2.R $(PATTERN) &> $@ && touch $@
$(motif_wga_ipdratios): get_motif_wga_ipdratio.R.log
	touch $@
get_motif_wga_ipdratio.R.log: $(occ_prefix).deep_kinetics_region.ab.bed.merged.sorted.slop20.bed.bed.merged_occ $(occ_prefix).deep_kinetics_region.cd.bed.merged.sorted.slop20.bed.bed.merged_occ
	Rscript ../get_motif_wga_ipdratio.R $(PATTERN) &> $@ && touch $@
motif_ipd.PD2182.csv: $(occ_prefix).deep_kinetics_region.PD2182.bed.merged.sorted.slop20.bed.bed.merged_occ
	Rscript ../get_motif_ipd.PD2182.v2.R $(PATTERN) &> get_motif_ipd.PD2182.R.log && touch $@
motif_ipd.PD2182sequel.csv: $(occ_prefix).deep_kinetics_region.PD2182sequel.bed.merged.sorted.slop20.bed.bed.merged_occ
...
$(motif_ipdratios): get_motif_ipdratio.R.log
	touch $@
get_motif_ipdratio.R.log: $(occ_prefix).deep_kinetics_region.k_normBy_ab.bed.slop20.bed.bed.merged_occ $(occ_prefix).deep_kinetics_region.l_normBy_cd.bed.slop20.bed.bed.merged_occ
	Rscript ../get_motif_ipdratio.v2.R $(PATTERN) &> $@ && touch $@

motif_ipdratio.PD2182.csv: $(occ_prefix).deep_kinetics_region.PD2182.bed.merged.sorted.slop20.bed.bed.merged_occ
	Rscript ../get_motif_ipdratio.PD2182.R $(PATTERN) &> get_motif_ipdratio.PD2182.R.log && touch $@
motif_ipdratio.PD2182sequel.csv: $(occ_prefix).deep_kinetics_region.PD2182sequel.bed.merged.sorted.slop20.bed.bed.merged_occ
	Rscript ../get_motif_ipdratio.PD2182sequel.R $(PATTERN) &> get_motif_ipdratio.PD2182sequel.R.log && touch $@

motif_ipdratio.k.csv: $(occ_prefix).deep_kinetics_region.k.bed.merged.sorted.slop20.bed.bed.merged_occ
	Rscript ../get_motif_ipdratio.k.R $(PATTERN) &> get_motif_ipdratio.k.R.log && touch $@
motif_ipdratio.l.csv: $(occ_prefix).deep_kinetics_region.l.bed.merged.sorted.slop20.bed.bed.merged_occ
	Rscript ../get_motif_ipdratio.l.R $(PATTERN) &> get_motif_ipdratio.l.R.log && touch $@

motif_ipd.k.csv: $(occ_prefix).deep_kinetics_region.k.bed.merged.sorted.slop20.bed.bed.merged_occ
	Rscript ../get_motif_ipd.k.v2.R $(PATTERN) &> get_motif_ipd.k.R.log && touch $@
motif_ipd.l.csv: $(occ_prefix).deep_kinetics_region.l.bed.merged.sorted.slop20.bed.bed.merged_occ
	Rscript ../get_motif_ipd.l.v2.R $(PATTERN) &> get_motif_ipd.l.R.log && touch $@

motif_ipd.abcd.csv: $(occ_prefix).deep_kinetics_region.abcd.bed.merged.sorted.slop20.bed.bed.merged_occ
	Rscript ../get_motif_ipd.abcd.v2.R $(PATTERN) &> get_motif_ipd.abcd.R.log && touch $@
motif_ipd.kl.csv: $(occ_prefix).deep_kinetics_region.kl.bed.merged.sorted.slop20.bed.bed.merged_occ
	Rscript ../get_motif_ipd.kl.v2.R $(PATTERN) &> get_motif_ipd.kl.R.log && touch $@
motif_ipdratio.abcd.csv: $(occ_prefix).deep_kinetics_region.abcd.bed.merged.sorted.slop20.bed.bed.merged_occ
	Rscript ../get_motif_ipdratio.abcd.R $(PATTERN) &> get_motif_ipdratio.abcd.R.log && touch $@
motif_ipdratio.kl.csv: $(occ_prefix).deep_kinetics_region.kl.bed.merged.sorted.slop20.bed.bed.merged_occ
	Rscript ../get_motif_ipdratio.kl.R $(PATTERN) &> get_motif_ipdratio.kl.R.log && touch $@
motif_ipdratio.kl_normBy_abcd.csv: $(occ_prefix).deep_kinetics_region.kl_normBy_abcd.bed.slop20.bed.bed.merged_occ
...
$(motif_kinetics_csv_celegans): %.c_elegans.csv: %.csv
	...
	Rscript ../extract_celegans_data.R $< $(occ_prefix).deep_kinetics_region.$${sample}.bed*.slop20.bed.bed.merged_occ $@

$(motif_kinetics_csv_ecoli): %.e_coli.csv: %.csv
	sample=$$(echo $< | sed -E "s/motif[_0-9a-zA-Z]+\.([_0-9a-zA-Z]+)\.csv/\1/"); \
	Rscript ../extract_ecoli_data.R $< $(occ_prefix).deep_kinetics_region.$${sample}.bed*.slop20.bed.bed.merged_occ $@
