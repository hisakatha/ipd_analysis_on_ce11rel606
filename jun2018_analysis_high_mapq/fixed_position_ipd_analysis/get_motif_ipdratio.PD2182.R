...
PD2182_occ <- fread("motif_occ.deep_kinetics_region.PD2182.bed.merged.sorted.slop20.bed.bed.merged_occ", sep = " ", col.names = merged_occ_col)

reference_skip <- c(0, cumsum(width(reference)) * 2)
names(reference_skip) <- names(reference)

get_motif_ipdratio(PD2182_data, PD2182_occ, "motif_ipdratio.PD2182.csv", reference_skip)
