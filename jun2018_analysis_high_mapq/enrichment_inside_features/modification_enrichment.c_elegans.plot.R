...
d <- fread("modification_enrichment.c_elegans.csv")
d <- d[region != "ALL"]
d <- d[base != "ALL"]
...
pdf("modification_enrichment.c_elegans.plot.all_samples.pdf", height = 8, width = 12, onefile = TRUE)
plot.enrichment.dual_facet.all_sample.v2(d, ylab_text)
invisible(dev.off())

pdf("modification_enrichment.c_elegans.plot.all_samples.subset1.pdf", height = 6, width = 5, onefile = TRUE)
plot.enrichment.dual_facet.all_sample.subset3.v2(d, ylab_text)
invisible(dev.off())
