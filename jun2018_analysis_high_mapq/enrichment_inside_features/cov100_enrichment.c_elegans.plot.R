...
d <- fread("cov100_enrichment.c_elegans.csv")
d <- d[region != "ALL"]
#d <- d[base != "ALL"]
...
ylab_text <- "Fold change of fraction of bases with coverage 100 or more in each region"
...
d <- d[sample != "abcd"]
pdf("cov100_enrichment.c_elegans.plot.all_samples.pdf", height = 8, width = 15, onefile = TRUE)
plot.enrichment.dual_facet.all_sample.v2(d, ylab_text)
invisible(dev.off())

pdf("cov100_enrichment.c_elegans.plot.all_samples.subset1.pdf", height = 6, width = 6, onefile = TRUE)
