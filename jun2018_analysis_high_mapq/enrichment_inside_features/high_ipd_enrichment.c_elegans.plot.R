...
d <- fread("high_ipd_enrichment.c_elegans.csv")
d <- d[region != "ALL"]
#d <- d[base != "ALL"]
...
ylab_text <- "Fold change of fraction of high IPD bases in each region"
...
pdf("high_ipd_enrichment.c_elegans.plot.all_samples.subset1.pdf", height = 6, width = 6, onefile = TRUE)
