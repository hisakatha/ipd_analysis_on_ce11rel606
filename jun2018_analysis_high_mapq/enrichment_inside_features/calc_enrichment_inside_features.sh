#!/usr/bin/env bash
# Input loci (.sorted.gff)
INPUT="$1"
INPUT_BASE="$(basename $INPUT .gff)"
...
all_input_nums=($(get_gff_base_nums $INPUT))
...
calc_enrichment_in_a_feature () {
    ...
    bedtools intersect $STRAND_FLAG -sorted -g $FAI -a $INPUT -b $FEATURE_FILE | bedtools sort -faidx $FAI | bedtools merge -s -c 9,6,7 -o distinct,mean,distinct > $INPUT_BASE.inside.$FEATURE_ID
