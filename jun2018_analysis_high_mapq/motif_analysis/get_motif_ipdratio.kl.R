...
kl_occ <- fread(paste0(motif, ".deep_kinetics_region.kl.bed.merged.sorted.slop20.bed.fa.bed.merged_occ"), sep = " ", col.names = merged_occ_col)

reference_skip <- c(0, cumsum(width(reference)) * 2)
names(reference_skip) <- names(reference)

get_motif_ipd(kl_data, kl_occ, "motif_ipdratio.kl.csv", reference_skip)
