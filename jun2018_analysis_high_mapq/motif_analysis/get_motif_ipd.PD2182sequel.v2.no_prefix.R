...
PD2182sequel_occ <- fread(paste0("motif_occ.deep_kinetics_region.PD2182sequel.bed.merged.sorted.slop20.bed.fa.bed.merged_occ"), sep = " ", col.names = merged_occ_col)

reference_skip <- c(0, cumsum(width(reference)) * 2)
names(reference_skip) <- names(reference)

get_motif_ipd(PD2182sequel_data, PD2182sequel_occ, "motif_ipd.PD2182sequel.csv", reference_skip)
