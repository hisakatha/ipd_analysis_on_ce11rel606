...
ref_celegans=$(cat $MOTIF.ce11rel606.bed.merged_occ | (grep -v "$ecoli" || [[ $? == 1 ]]) | wc -l)
ref_ecoli=$(cat $MOTIF.ce11rel606.bed.merged_occ | (grep "$ecoli" || [[ $? == 1 ]]) | wc -l)
...
do
    ...
    deep_celegans=$(cat $MOTIF.deep_kinetics_region.$ID.bed.merged.sorted.slop20.bed.fa.bed.merged_occ | (grep -v "$ecoli" || [[ $? == 1 ]]) | wc -l)
    deep_ecoli=$(cat $MOTIF.deep_kinetics_region.$ID.bed.merged.sorted.slop20.bed.fa.bed.merged_occ | (grep "$ecoli" || [[ $? == 1 ]]) | wc -l)
    ...
    high_celegans=$(cat $MOTIF.extreme_ipd.high.$ID.coverage25.slop20.fullLength.gff.fa.bed.merged_occ | (grep -v "$ecoli" || [[ $? == 1 ]]) | wc -l)
    ...
    low_celegans=$(cat $MOTIF.extreme_ipd.low.$ID.coverage25.slop20.fullLength.gff.fa.bed.merged_occ | (grep -v "$ecoli" || [[ $? == 1 ]]) | wc -l)
