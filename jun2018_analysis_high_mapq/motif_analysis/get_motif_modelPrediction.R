...
get_motif_ipd <- function(data, occ, out, ref_skips){
    ...
    table_list <- foreach(i = 1:occ_n) %dopar% {
        ...
        out_values <- c(rep(0, (collect_start - index_start) * 2), data[ref_skips[occ_chr] + (collect_start * 2 + 1):(collect_end * 2 + 2), modelPrediction], rep(0, (index_end - collect_end) * 2))
...
ab_occ <- fread(paste0(motif, ".deep_kinetics_region.ab.bed.merged.sorted.slop20.bed.fa.bed.merged_occ"), sep = " ", col.names = merged_occ_col)
cd_occ <- fread(paste0(motif, ".deep_kinetics_region.cd.bed.merged.sorted.slop20.bed.fa.bed.merged_occ"), sep = " ", col.names = merged_occ_col)

reference_skip <- c(0, cumsum(width(reference)) * 2)
names(reference_skip) <- names(reference)

get_motif_ipd(ab_data, ab_occ, "motif_modelPrediction.ab.csv", reference_skip)
get_motif_ipd(cd_data, cd_occ, "motif_modelPrediction.cd.csv", reference_skip)
