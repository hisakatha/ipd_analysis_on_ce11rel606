...
"GCGCGTCA", "GTGTGTGY",
"TACCCCKA", "TACCTTGA")
motifs_low_and_high <- c("TGACGTCA")
motifs_low_ipd <- c("ACATMTGG", "CTGDAR", "TACTGTAG", motifs_low_ipd, motifs_low_and_high)

#subset1_motifs <- c("GAGG", "AGAA", "GATC", "ACGCRTG", "ATCAGCTG", "GGN_4",
...
subset1_motifs <- c("RGTA", "AAAABT", "AGAGAGTA", "AGGCAGGC", "ATGGGAYA", "ATTGTTAC",
                    "CAAACTAC", "CCAATCAG", "DGCTTC", "YGGAR",
                    motifs_low_ipd)
subset1_positions <- c(4, 2, 1, 1, 6, 1, 3, 3, 2, 4,
                       6, 2, 4, 3, 4, 6, 2, 6, 6, 1, 1, 1)
subset1_strand <- "+"
...
foreach_ret <- foreach(i = 1:length(subset1_motifs)) %do% {
    ...
    output <- sprintf("motif_kinetics_statistical_tests.rank_sum.subset3.%s.csv", motif_dir)
...
fwrite(rbindlist(foreach_ret), file = "motif_kinetics_statistical_tests.rank_sum.subset3.csv")
