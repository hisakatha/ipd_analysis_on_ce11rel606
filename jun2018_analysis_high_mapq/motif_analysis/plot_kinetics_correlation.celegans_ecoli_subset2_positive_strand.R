...
ipd_table <- Reduce(function(x, y) merge(x, y, all = TRUE, by = c("position", "strand")), list(ab_mean, cd_mean, k_mean, l_mean, ab_mean_ecoli, cd_mean_ecoli, k_mean_ecoli, l_mean_ecoli))
...
ipd_table3 <- Reduce(function(x, y) merge(x, y, all = TRUE, by = c("position", "strand")), list(ab_mean, cd_mean, k_mean, l_mean))
ipd_table3[, c("position", "strand") := NULL]

# function: corrplot.na
source("../plot_kinetics_correlation.corrplot.na.R")

pdf("plot_kinetics_correlation.celegans_ecoli_subset2_positive_strand.pdf", onefile = TRUE, width = 8, height = 8)
