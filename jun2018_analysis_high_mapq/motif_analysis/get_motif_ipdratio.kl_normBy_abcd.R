...
source("../../load_jun2018_data.saved.fst.kl_normBy_abcd.R", chdir = TRUE)

#bed_col <- c("chr", "start", "end", "name", "score", "strand")
merged_occ_col <- c("chr", "start", "strand")

kl_normBy_abcd_occ <- fread(paste0(motif, ".deep_kinetics_region.kl_normBy_abcd.bed.slop20.bed.fa.bed.merged_occ"), sep = " ", col.names = merged_occ_col)

reference_skip <- c(0, cumsum(width(reference)) * 2)
names(reference_skip) <- names(reference)

get_motif_ipdratio(kl_normBy_abcd_data, kl_normBy_abcd_occ, "motif_ipdratio.kl_normBy_abcd.csv", reference_skip)
