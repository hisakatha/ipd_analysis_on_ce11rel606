...
source("../../load_and_save_jun2021_pd1074_ccs_data.saved.fst.R", chdir = TRUE)

#bed_col <- c("chr", "start", "end", "name", "score", "strand")
merged_occ_col <- c("chr", "start", "strand")

jun2021_pd1074_ccs_occ <- fread(paste0(motif, ".deep_kinetics_region.jun2021_pd1074_ccs.bed.merged.sorted.slop20.bed.fa.bed.merged_occ"), sep = " ", col.names = merged_occ_col)

reference_skip <- c(0, cumsum(width(reference)) * 2)
names(reference_skip) <- names(reference)

get_motif_ipd(jun2021_pd1074_ccs_data, jun2021_pd1074_ccs_occ, "motif_ipd.jun2021_pd1074_ccs.csv", reference_skip)
