...
plot_motif_kinetics <- function(kinetics, estimate, title_text, motif_string, sample_name, ylab_text, global_mean, global_var, global_size){
    kinetics_occ <- ifelse(kinetics[, .N] == 0, 0, kinetics[strand == "+", length(unique(src))])
    estimate_occ <- ifelse(estimate[, .N] == 0, 0, estimate[strand == "+", length(unique(src))])
    occ_threshold <- 100
...
plot_motif_kinetics_comparison <- function(kinetics1, kinetics2, type1, type2, title_text, motif_string, sample_name, ylab_text, global_mean, global_var, global_size){
    kinetics1_occ <- ifelse(kinetics1[, .N] == 0, 0, kinetics1[strand == "+", length(unique(src))])
    kinetics2_occ <- ifelse(kinetics2[, .N] == 0, 0, kinetics2[strand == "+", length(unique(src))])
    occ_threshold <- 100
