...
"GTGTGTGY",
"TACCCCKA", "TACCTTGA")
motifs_low_ipd <- c("ACATMTGG", "CTGDAR", "TACTGTAG", motifs_low_ipd)
...
subset1_motifs <- motifs_low_ipd
# low IPD position: c(6, 2, 4, 3, 4, 6, 2, 6, 6, 1, 1, 1)
subset1_positions <- c(1, 5, 2, 1, 3, 6, 1, 8, 8)
subset1_strand <- "+"
...
foreach_ret <- foreach(i = 1:length(subset1_motifs)) %do% {
    ...
    output <- sprintf("motif_kinetics_statistical_tests.rank_sum.subset4.%s.csv", motif_dir)
...
fwrite(rbindlist(foreach_ret), file = "motif_kinetics_statistical_tests.rank_sum.subset4.csv")
